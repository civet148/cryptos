...
import (
	"crypto/rsa"
	"fmt"
	"github.com/civet148/cryptos/gorsa"
)
...
func TestPriKeyEncryptAndPubKeyDecrypt() {
	...
	prikey, err = gorsa.GetPriKey([]byte(Pirvatekey))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	pubkey, err = gorsa.GetPubKey([]byte(Pubkey))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	encData, errEnc := gorsa.PriKeyEncrypt(prikey, []byte(plainText))
	if errEnc != nil {
		fmt.Println(errEnc)
		return
	}

	decData, errDec := gorsa.PubKeyDecrypt(pubkey, encData)
	if errDec != nil {
		fmt.Println(errDec)
		return
	}
...
func TestPubKeyEncryptAndPriKeyDecrypt() {
	...
	fmt.Println("加密前字符串 [", plainText, "] 长度", len(plainText))
	encData, errEnc := gorsa.PubKeyEncrypt(pubkey, []byte(plainText))
	if errEnc != nil {
		fmt.Println(errEnc)
		return
	}

	decData, errDec := gorsa.PriKeyDecrypt(prikey, encData)
	if errDec != nil {
		fmt.Println(errDec)
		return
	}
...
func TestPriKeyEncryptAndPubKeyDecryptByFile() {
	...
	//读取私钥证书文件test.pfx用于加密数据
	prikey, err = gorsa.GetPriKeyByPfxFile("test.pfx", "123456")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	//读取私钥证书文件test.cer用于密数据
	pubkey, err = gorsa.GetPubKeyByCerFile("test.cer")
	if err != nil {
		fmt.Println(err)
		return
	}
